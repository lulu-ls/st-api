...
const OperationGameActivityList = "/api.game.v1.Game/ActivityList"
...
const OperationGameDailyTask = "/api.game.v1.Game/DailyTask"
...
type GameHTTPServer interface {
	...
	// DailyTask 每日任务
	DailyTask(context.Context, *DailyTaskRequest) (*DailyTaskReply, error)
	// GameInfo 获取房间信息
...
func RegisterGameHTTPServer(s *http.Server, srv GameHTTPServer) {
	...
	r.POST("/st-games/v1/game/daily/task", _Game_DailyTask0_HTTP_Handler(srv))
}
...
func _Game_ActivityList0_HTTP_Handler(srv GameHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		...
		http.SetOperation(ctx, OperationGameActivityList)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
...
func _Game_DailyTask0_HTTP_Handler(srv GameHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in DailyTaskRequest
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		if err := ctx.BindQuery(&in); err != nil {
		...
		http.SetOperation(ctx, OperationGameDailyTask)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.DailyTask(ctx, req.(*DailyTaskRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*DailyTaskReply)
		return ctx.Result(200, reply)
	}
}
...
type GameHTTPClient interface {
	...
	DailyTask(ctx context.Context, req *DailyTaskRequest, opts ...http.CallOption) (rsp *DailyTaskReply, err error)
...
func (c *GameHTTPClientImpl) ActivityList(ctx context.Context, in *ActivityListRequest, opts ...http.CallOption) (*ActivityListReply, error) {
	var out ActivityListReply
	pattern := "/st-games/v1/game/activity/list"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationGameActivityList))
...
func (c *GameHTTPClientImpl) DailyTask(ctx context.Context, in *DailyTaskRequest, opts ...http.CallOption) (*DailyTaskReply, error) {
	var out DailyTaskReply
	pattern := "/st-games/v1/game/daily/task"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation(OperationGameDailyTask))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "POST", path, in, &out, opts...)
	if err != nil {
		return nil, err
